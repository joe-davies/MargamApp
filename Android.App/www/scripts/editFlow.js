...
let app = {
    ...
    takephoto1: function takephoto1(imgIndex) {
        let _cameraOptions = app.cameraOptions();
    ...
    selectphoto1: function selectphoto1(imgIndex) {
        let _photoOptions = app.photoLibraryOptions();
    ...
    saveData: function saveData() {
        try {
            ...
            let _sampleId = parseInt($("#lblSampleId").text());
            let _editedLoadData = [];
            let _isDataFound = false;
            let _existingEdittedRecord = {};

            if (window.localStorage.hasOwnProperty('editedLoadEntryData')) {
                let editedData = window.localStorage.getItem('editedLoadEntryData');
                let editedFloData = JSON.parse(editedData);
            ...
            if (_isDataFound == false) {
                ...
                let cData = window.localStorage.getItem('cloudLoadEntryData');
                let clodData = JSON.parse(cData);
...
function fnFillPlantOperators(oLoad) {
    let _plantOperators = window.localStorage.getItem('plantOperatorsData');
    if (_plantOperators.length > 0) {
        let plantOperatorData = JSON.parse(_plantOperators);
        let _pOperatorOptions = "";
        ...
        let plantOperatorSelect = $("#operatorName");
...
function fnFillRejectionReasonData(oLoad) {
    let _rejectionReasons = window.localStorage.getItem('loadRejectionReasonData');
    if (_rejectionReasons.length > 0) {
        let rejectionReasonData = JSON.parse(_rejectionReasons);
        let _rReasonOptions = "";
        ...
        if (isMuliRejectionReason) {
            ...
            let rejectReasonSelect = $("#rejectionReason");
            rejectReasonSelect.append(_rReasonOptions);
            rejectReasonSelect.selectmenu();
            rejectReasonSelect.selectmenu('refresh', true);
        }
...
function fnFillPlantOperatorData(oLoad) {
    let _plantOperator = window.localStorage.getItem('plantOperatorsData');
    if (_plantOperator.length > 0) {
        let plantOperatorData = JSON.parse(_plantOperator);
        let _plantOperatorOptions = "<option value='-1'>Select Plant Operator</option>";
...
function fnGetDropdownValues(oLoad) {
    ...
    let _plantOperatorIndex = $('#operatorName option:selected').val();
    ...
    if (isMuliRejectionReason) {
        ...
        let _rejReasonIndex = $('#rejectionReason option:selected').val();
        if (_plantOperatorIndex == -1) {
            oLoad.rejReasons = null;
        }
        else {
...
$(document).ready(function () {
    ...
    /// Task #12 
    ///Bale location selector should be an image selector #12
    $(".tdBale").on("click", function () {
        var _eleHidden = $(this).find('input[type=hidden]');
        var _previousValue = _eleHidden.val();
